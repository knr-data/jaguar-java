...
public class RequestOptions {
  ...
  /** Stripe version always set at {@link Jaguar#API_VERSION}. */
  private final String stripeVersion = Jaguar.API_VERSION;
  /**
   * Stripe version override when made on behalf of others. This can be used when the returned
   * response will not be deserialized into the current classes pinned to {@link Jaguar#VERSION}.
   */
  ...
  public static RequestOptions getDefault() {
    return new RequestOptions(
        ...
        null,
        null,
        Jaguar.getConnectTimeout(),
        Jaguar.getReadTimeout(),
        Jaguar.getMaxNetworkRetries(),
        Jaguar.getConnectionProxy(),
        Jaguar.getProxyCredential());
  }
  ...
  public static final class RequestOptionsBuilder {
    ...
    public RequestOptionsBuilder() {
      this.apiKey = Jaguar.apiKey;
      this.clientId = Jaguar.clientId;
      this.connectTimeout = Jaguar.getConnectTimeout();
      this.readTimeout = Jaguar.getReadTimeout();
      this.maxNetworkRetries = Jaguar.getMaxNetworkRetries();
      this.connectionProxy = Jaguar.getConnectionProxy();
      this.proxyCredential = Jaguar.getProxyCredential();
    }
     ...
     * com.jaguar.model.EphemeralKey#create(Map, RequestOptions)}. Setting this value in a typical
     * scenario will result in deserialization error as the model classes have schema according to
     * the pinned {@link Jaguar#API_VERSION} and not the {@code stripeVersionOverride}
     *
