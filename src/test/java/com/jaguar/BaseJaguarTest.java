...
public class BaseJaguarTest {
  private static final String MOCK_MINIMUM_VERSION = "0.109.0";

  private static String port;

  public static JaguarResponseGetter networkSpy;
  ...
  public void setUpStripeMockUsage() {
    ...
    this.origClientId = Jaguar.clientId;

    Jaguar.overrideApiBase("http://localhost:" + port);
    Jaguar.overrideUploadBase("http://localhost:" + port);
    Jaguar.apiKey = "sk_test_123";
    Jaguar.clientId = "ca_123";
    Jaguar.enableTelemetry = false;

    networkSpy = Mockito.spy(new LiveJaguarResponseGetter());
  ...
  public void tearDownStripeMockUsage() {
    ...
    Jaguar.overrideUploadBase(this.origUploadBase);
    Jaguar.apiKey = this.origApiKey;
    Jaguar.clientId = this.origClientId;
  }

  /**
   * {@code params} and {@code options} defaults to {@code null}.
   *
   * @see BaseJaguarTest#verifyRequest(ApiResource.RequestMethod, String, Map, RequestOptions)
   */
   ...
   */
  public static <T extends JaguarObjectInterface> void verifyRequest(
    ...
    String url;
    if (path.startsWith("/")) {
      url = String.format("%s%s", Jaguar.getApiBase(), path);
    } else {
      url = path;
    }
   ...
   * {@code params} and {@code options} defaults to {@code null}.
   *
   * @see BaseJaguarTest#stubRequest(ApiResource.RequestMethod, String, Map, RequestOptions, Class,
   *     String)
   */
  public static <T extends JaguarObjectInterface> void stubRequest(
  ...
  public static <T extends JaguarObjectInterface> void stubOAuthRequest(
      Class<T> clazz, String response) throws JaguarException {
    Mockito.doReturn(ApiResource.GSON.fromJson(response, clazz))
        .when(networkSpy)
        .oauthRequest(
